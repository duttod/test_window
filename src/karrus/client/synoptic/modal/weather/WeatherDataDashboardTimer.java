...
/**
 * Creates a timer to plot weather data in a modal.
 * 
 * @author estelle.dumas@karrus-its.com
 */
public class WeatherDataDashboardTimer extends Timer implements DashBoardTimerInterface {
	 ...
	 */
	public WeatherDataDashboardTimer(String station, int refreshingRate, int timeHorizon) {
		dataToPlot2TimeSeries.put(Language.airTemperature, new TimeSeries());
		dataToPlot2TimeSeries.put(Language.dewTemperature, new TimeSeries());
		dataToPlot2TimeSeries.put(Language.roadTemperature, new TimeSeries());
		dataToPlot2TimeSeries.put(Language.belowSurfTemperature, new TimeSeries());
		dataToPlot2TimeSeries.put(Language.freezingTemperature, new TimeSeries());
...
////			final CountDataTimeSeriesPlot currentCountDataTimeSeriesPlot = iterator.next();
			...
			if (firstDateInCurrentCountDataTimeSeriesPlot==null && lastDateInCurrentCountDataTimeSeriesPlot==null){
				...
				Log.debug("CAS 11111");
				FrontalWebApp.genericDatabaseService.getWeatherDataForStation(station, timeHorizonStart, timeHorizonStop, new AsyncCallback<List<WthMeteoData>>() {
					@Override
					public void onFailure(Throwable caught) {
						Log.error("service.getWeatherDataForStation: failure\n"+caught.getMessage());
				...
				if (timeHorizonStart.after(firstDateInCurrentCountDataTimeSeriesPlot)){
					if (timeHorizonStop.after(lastDateInCurrentCountDataTimeSeriesPlot)){
						...
						if (lastDateInCurrentCountDataTimeSeriesPlot.after(timeHorizonStart)){
							...
							FrontalWebApp.genericDatabaseService.getWeatherDataForStation(station, lastDateInCurrentCountDataTimeSeriesPlot, timeHorizonStop, new AsyncCallback<List<WthMeteoData>>() {
								@Override
								public void onFailure(Throwable caught) {
									Log.error("genericDatabaseService.getWeatherDataForStation: failure\n"+caught.getMessage());
									new CustomDialogBox(Language.genericErrorString, Language.okString);
								}
								@Override
								public void onSuccess(List<WthMeteoData> result) {
...
//													currentSensorPlotInfo.getTimeSeries().setTimesSeries(timeSeries);
					...
					if (timeHorizonStop.after(firstDateInCurrentCountDataTimeSeriesPlot)){
						...
						if (timeHorizonStop.after(lastDateInCurrentCountDataTimeSeriesPlot)){
							...
							FrontalWebApp.genericDatabaseService.getWeatherDataForStation(station, timeHorizonStart, firstDateInCurrentCountDataTimeSeriesPlot, new AsyncCallback<List<WthMeteoData>>() {
								@Override
								public void onFailure(Throwable caught) {
									Log.error("genericDatabaseService.getWeatherDataForStation: failure\n"+caught.getMessage());
									new CustomDialogBox(Language.genericErrorString, Language.okString);
								...
								@Override
								public void onSuccess(final List<WthMeteoData> result1) {
									...
									FrontalWebApp.genericDatabaseService.getWeatherDataForStation(station, lastDateInCurrentCountDataTimeSeriesPlot, timeHorizonStop, new AsyncCallback<List<WthMeteoData>>() {
										...
										@Override
										public void onSuccess(List<WthMeteoData> result2) {
											for (String data : dataToPlot2TimeSeries.keySet()){
												try {
	...
	public TimeSeries getTimeSeries(List<WthMeteoData> weatherData, String columnName) throws Exception {
		List<Date> dates = new ArrayList<Date>();
		List<Double> values = new ArrayList<Double>();
		for (WthMeteoData data : weatherData){
			dates.add(data.getId().getTimestamp());
			if (columnName.equals(Language.airTemperature))
				values.add(data.getTemperatureAir());
			else if (columnName.equals(Language.dewTemperature))
				values.add(data.getTemperatureDew());
			else if (columnName.equals(Language.roadTemperature))
				values.add(data.getTemperatureRoad());
			else if (columnName.equals(Language.belowSurfTemperature))
				values.add(data.getTemperatureBelowSurf());
			else if (columnName.equals(Language.freezingTemperature))
				values.add(data.getTemperatureFreezing());
			else
				values.add(null);
		}
	...
	private PlotClickMessageFormatter getPlotClickMessageFormatter(String columName) {
		if (columName.equals(Language.airTemperature)) {
		...
		else if (columName.equals(Language.dewTemperature)) {
			PlotClickMessageFormatter plotClickMessageFormatter = new PlotClickMessageFormatter() {
				@Override
				public String getMessage(Plot plot, PlotPosition position, PlotItem item) {
					Date d = new Date((long)item.getDataPoint().getX());
		...
		else if (columName.equals(Language.roadTemperature)) {
			PlotClickMessageFormatter plotClickMessageFormatter = new PlotClickMessageFormatter() {
				@Override
				public String getMessage(Plot plot, PlotPosition position, PlotItem item) {
					Date d = new Date((long)item.getDataPoint().getX());
		...
		else if (columName.equals(Language.belowSurfTemperature)) {
			PlotClickMessageFormatter plotClickMessageFormatter = new PlotClickMessageFormatter() {
				@Override
				public String getMessage(Plot plot, PlotPosition position, PlotItem item) {
					Date d = new Date((long)item.getDataPoint().getX());
